...
def run_analysis(year=2025, min_underdog_matches=8):
    ...
    # Sort by total Upset Win Count, then by Underdog Match Win Rate
    upset_leaderboard.sort(key=lambda x: (x['Upset Win Count'], x['Underdog Match Win Rate (%)']), reverse=True)
    ...
    with open(csv_name, mode='w', newline='') as f:
        fieldnames = ['Team', 'Upset Win Count', 'Underdog Match Count', 'Underdog Match Win Rate (%)', 'Upset Frequency (%)', 'Total Match Count']
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(upset_leaderboard)

    # 4. Print Results
    print(f"\n--- TOP 10 BRACKET BUSTERS {year} ---")
    print(f"{'Rank':<5} {'Team':<8} | {'Wins':<5} | {'Underdog Match Win Rate':<25} | {'Upset Frequency'}")
    print("-" * 65)
    for i, row in enumerate(upset_leaderboard[:10]):
        print(f"{i+1:<5} {row['Team']:<8} | {row['Upset Win Count']:<5} | {row['Underdog Match Win Rate (%)']:<24}% | {row['Upset Frequency (%)']}%")
